...
export async function POST(
  ...
  try {
    ...
    const inscripcion = await prisma.inscripcion.findUnique({
      where: { cursoId_estudianteId: { cursoId: params.id, estudianteId: params.estudianteId } },
      include: { estudiante: true },
    });
    if (!inscripcion) throw new ErrorAcceso("Ese estudiante no está matriculado en este curso", 404);
